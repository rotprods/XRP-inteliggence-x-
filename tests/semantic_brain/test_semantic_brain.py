...
from xrp_regime_engine.semantic_brain.models import (
    ClaimRecord, EntityRecord, EntityType, EpistemicStatus, SourceTier, TemporalEnvelope,
)
...
def test_fact_contradiction_degrades_both_claims() -> None:
    ...
    b = ledger.append(ClaimRecord.from_statement("Rail did not select XRP", status=EpistemicStatus.FACT))
...
def test_entity_alias_collision_fails_closed() -> None:
    ...
    resolver.register(EntityRecord(entity_id="org:ripple", entity_type=EntityType.ORGANIZATION, canonical_name="Ripple"))
    with pytest.raises(ValueError, match="ambiguous"):
        resolver.register(EntityRecord(entity_id="asset:ripple", entity_type=EntityType.ASSET, canonical_name="Other", aliases={"Ripple"}))


def test_regime_context_excludes_future_information_and_zero_weights_narrative() -> None:
    records = [
        {"claim_id": "c1", "statement": "known", "status": "FACT", "evidence_confidence": 0.9, "available_at": NOW - timedelta(minutes=1), "source_ids": ["s1"]},
        {"claim_id": "c2", "statement": "future", "status": "FACT", "evidence_confidence": 0.9, "available_at": NOW + timedelta(minutes=1), "source_ids": ["s2"]},
        {"claim_id": "c3", "statement": "riddle", "status": "CLAIM_ONLY", "evidence_confidence": 0.1, "available_at": NOW - timedelta(minutes=1), "source_ids": ["social"]},
    ]
...
def test_prediction_cannot_be_scored_before_horizon() -> None:
    prediction = FrozenPrediction(
        prediction_id="p1", source_id="social:example", original_text="red october",
        published_at=NOW, frozen_at=NOW + timedelta(minutes=1), target="XRP > 3",
        horizon_end=NOW + timedelta(days=30), success_rule="close > 3", failure_rule="otherwise",
    )
    with pytest.raises(ValueError, match="horizon"):
        prediction.resolve(PredictionResolution.HIT, NOW + timedelta(days=1), ["market:snapshot"])
